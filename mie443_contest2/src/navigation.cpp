...
void Navigation::getViewPoints(std::vector<std::vector<float>> coords) {
    ...
    ROS_INFO("Finished obtaining %d view points", num_view_points);
}
...
void Navigation::traverseBox(int box_idx) {
    ...
    if(it == box_view_points.end()) {
        ...
        float dist_last = getDist(curr_pos, view_points[end_idx-1]);
        ...
        ROS_INFO("Traversed box %d, %d", start_idx, end_idx);
        // start traversing 
